    ...
    if not request.is_websocket():#判断是不是websocket连接
        ...
        while 1:
            try:
                ...
                pull_posts1 = TiebaPost.objects(Q(st_time__gte=cur)&Q(should_pull=True)).order_by("-st_time")
                pull_posts2 = Post.objects(Q(st_time__gte=cur)&Q(should_pull=True)).order_by("-st_time")
                # pull_posts1 = TiebaPost.objects(Q(should_pull=True))
                # pull_posts2 = Post.objects(Q(should_pull=True))
                # pull_posts1 = TiebaPost.objects(Q(should_pull=1))
                # pull_posts2 = Post.objects(Q(should_pull=1))
